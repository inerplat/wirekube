...
| `routing.localSubnetPolicy` | string | No | `bypass` | `bypass` keeps traffic to a peer on the observer's own segment on the physical link instead of the tunnel. Suppression requires cryptographic proof (a live WireGuard handshake whose direct endpoint is the advertised address inside the same attached prefix), so VPCs that reuse one private range keep their tunnel routes. `tunnel` encrypts same-segment traffic too. |
| `routing.excludeCIDRs` | []string | No | - | Destinations no agent may install into the WireKube table, regardless of reachability. Matching is by containment in route units; entries override gateway routes they fully contain, but can never suppress mesh overlay routes. Route installation only — inbound traffic admitted by AllowedIPs is unaffected. |
