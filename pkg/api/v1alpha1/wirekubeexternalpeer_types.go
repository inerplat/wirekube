...
type WireKubeExternalPeerSpec struct {
	...
	// to reach through the mesh. Used to populate the AllowedIPs field of the
	// rendered WireGuard conf and to scope ingress-side routing.
	//
	// An explicit list REPLACES the controller defaults rather than adding to
	// them, so it must name every range the peer needs — including the mesh
	// overlay and pod CIDRs it would otherwise get for free. Leave it empty
	// unless the peer really should reach a narrower scope than the cluster.
	// +optional
...
type WireKubeExternalPeerStatus struct {
	...
	// when the operator set one explicitly; otherwise the controller fills it
	// with the mesh overlay CIDR, every Node's pod CIDR(s), the Service
	// ClusterIP range(s) and every WireKubeGateway route, sorted, so the peer
	// reaches the same cluster an in-cluster peer reaches. The CLI renders
	// from this field rather than the spec so its output is authoritative
	// regardless of whether the operator overrode the defaults.
	// +optional
	AllowedDestinations []string `json:"allowedDestinations,omitempty"`
