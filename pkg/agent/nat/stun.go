...
// the NAT effectively allocates ports randomly (symmetric NATs have been
// observed producing 26 000-58 000 increments), and a sequential
...
func DiscoverPublicEndpointWithNATType(ctx context.Context, localPort int, stunServers []string) (*STUNResult, error) {
	...
	if allSame {
		...
		// public IP attached directly to the NIC (Oracle Cloud shapes and
		// similar) land here, and mislabelling them as "cone" would trigger
