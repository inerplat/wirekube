...
// The leg counters answer "what did this node put on each leg", so a write
// that failed must not be counted. Charging them at the decision point
// inflated both legs during exactly the outages the metrics exist to
// diagnose.
func TestSendLegCountersFollowWriteResults(t *testing.T) {
	clock := newFakeClock()
	p := newHBPeer(t, clock, false)
	peer := mustKeyPair(t)
	key := peer.PublicKeyBase64()

	// A direct address nothing listens on still accepts local writes, so use
	// a closed socket's address to force a local send error instead.
	...
	p.b.SetPeerPath(key, PathModeDirect, deadAddr)
	pp := p.b.GetPeerPath(key)
	pp.DirectHealth.LastSeen.Store(clock.Now())

	before, _ := p.b.PeerPathStats(key)
	// Two buffers on a healthy direct leg: both counted.
	if err := p.b.Send([][]byte{{4, 0, 0, 0}, {4, 0, 0, 0}}, &WireKubeEndpoint{dst: deadAddr, relayPeerKey: relayPeerKey{peerKey: peer.Public}}); err != nil {
		t.Fatalf("Send: %v", err)
	}
	after, _ := p.b.PeerPathStats(key)
	sent := after.SentDirectOnly - before.SentDirectOnly
	if sent != 2 {
		t.Fatalf("direct_only delta = %d, want 2 (one per buffer)", sent)
	}
	if after.SentDual != before.SentDual || after.SentRelayOnly != before.SentRelayOnly {
		t.Fatalf("a direct-only send moved another leg's counter: dual+%d relay+%d",
			after.SentDual-before.SentDual, after.SentRelayOnly-before.SentRelayOnly)
	}
}
